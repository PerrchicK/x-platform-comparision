...
open class PerrFuncs {
    ...
    static func presentAppStoreRating() {
    ...
    class func copyToClipboard(textToCopy: String) {
    ...
    static func isRunningUnderDevelopmentEnvironment() -> Bool {
        return true // UtilsObjC.currentEnvironment() == BRZEnvironmentDevelopment
    }
    ...
    @discardableResult
    static func goToAppStore() -> Bool {
        if let appStoreUrl = URL(string: Configurations.Constants.iosAppStoreAddress), UIApplication.shared.canOpenURL(appStoreUrl) {
            if #available(iOS 10.0, *) {
            ...
            📕("Failed getting url: \(Configurations.Constants.iosAppStoreAddress)")
        }
...
extension Array {
    func toJsonString() -> String? {
        let _objectData: Data? = try? JSONSerialization.data(withJSONObject: self, options: [])
    ...
    mutating func remove(where predicate: (Array.Iterator.Element) throws -> Bool) -> Element? {
        if let indexToRemove = try? self.index(where: predicate) {
...
extension DictionaryConvertible {
    ...
    func toDictionary() -> RawJsonFormat {
        ...
        guard let firebaseDictionary = try? JSONSerialization.jsonObject(with: objectData, options: JSONSerialization.ReadingOptions.allowFragments) as? [AnyHashable: Any] else { return [:] }
